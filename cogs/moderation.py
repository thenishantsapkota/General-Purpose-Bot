...
class Moderation(Cog):
    ...
    async def mute_command(self, ctx, members:Greedy[Member], time:TimeConverter, *, reason:Optional[str]= "No Reason Specified."):
        ...
        for member in members:
            if ctx.author.top_role > member.top_role:
                if muted_role not in member.roles:
                    ...
                    model, _ = await MuteModel.get_or_create(guild_id = guild.id, member_id = member.id, time = time, role_id = role_ids)
                    await model.save()
                    ...
                    embed.set_author(name=f"{author.name} # {author.discriminator} [ID {author.id}]", icon_url = author.avatar_url)
                ...
                await ctx.send("You cannot run moderation actions on the users on same rank as you or higher than you.",
                delete_after = 10)
        if len(unmutes):
            await asyncio.sleep(time)
            await self.unmute_handler(ctx, members)

    @command(name = "unmute", aliases = ["unsilence"],brief = "Unmute a member from the server.")
    @commands.has_permissions(manage_messages = True)
    async def unmute(self, ctx, members:Greedy[Member], *, reason:Optional[str]= "No reason provided."):
        if not len(members):
