"""Author of this code: https://github.com/falsedev"""
import warnings
...
from zorander.utils import fuzzy, rtfm
...
class RTFM(commands.Cog):
    ...
    async def list_targets(self, ctx: commands.Context):
        ...
        embed = Embed(title="RTFM list of avaliable modules", color=self.bot.color)
        embed.description = "\n".join(
            [
                "[{0}]({1}): {2}".format(
                    target,
