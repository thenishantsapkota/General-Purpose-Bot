...
class Warnings(Cog):
    ...
    async def warning_count(self, ctx: commands.Context, member: Member) -> None:
        ...
        if len(warn_model) in range(8, 10):
            await self.mod.mute_handler(ctx, [member], 21600, f"Warning Count : {len(warn_model)}")
    ...
    async def warn_command(
        ...
        for member in members:
            ...
            model = await WarningsModel.create(
                guild_id=guild.id, member_id=member.id, reason=reason, author_name = author, date = pretty_datetime(datetime.now())
            )
            await model.save()
            embed = Embed(
                color=Color.red(),
    ...
    async def warnings(self, ctx: commands.Context, member: Optional[Member]):
        ...
        embed.set_footer(text=f"Requested by {author.name} | Total Warnings: {len(warn_model)}")
        await ctx.send(embed=embed)


    @warnings.command(name="delete")
    async def warnings_delete(self, ctx: commands.Context, member: Optional[Member], id: int) -> None:
    ...
    async def warnings_clear(self, ctx: commands.Context, member: Member) -> None:
        ...
        log_channel = await self.permissions.log_channel_check(guild)
        model = await WarningsModel.filter(guild_id=guild.id, member_id=member.id).delete()
