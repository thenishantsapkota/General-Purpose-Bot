...
class Snipe(Cog):
    ...
    async def snipe_group(self, ctx: commands.Context) -> None:
        try:
            ...
            await ctx.send("You need to have a deleted message to snipe ya fool.", delete_after=10)
    ...
    async def snipe_edited(self, ctx: commands.Context):
        try:
            ...
            embed.set_author(
                name=f"Edited Message from - {after.author.display_name}", icon_url=after.author.avatar_url
            )
            embed.set_footer(text=f"Sniped by {ctx.author} | in #{ctx.channel.name}")
